@if /I "%~1"=="--ci-configure" goto ci_configure
@if /I "%~1"=="--ci-build" goto ci_build
...
:: GitHub Actions entry points. The legacy release mode above intentionally
:: keeps using configure_local_paths.cmd and its existing upload/sign flow.
:ci_configure
@echo off
setlocal

call :ci_require ZANO_SOURCE_DIR || exit /B 1
call :ci_require ZANO_BUILD_DIR || exit /B 1
call :ci_require ZANO_DEPS_DIR || exit /B 1
call :ci_require ZANO_CMAKE_ARCH || exit /B 1

set "CI_TESTNET=FALSE"
if /I "%ZANO_TESTNET%"=="true" set "CI_TESTNET=TRUE"
if "%ZANO_TESTNET%"=="1" set "CI_TESTNET=TRUE"

set "CI_BUILD_GUI=FALSE"
if /I "%ZANO_BUILD_GUI%"=="true" set "CI_BUILD_GUI=TRUE"
if "%ZANO_BUILD_GUI%"=="1" set "CI_BUILD_GUI=TRUE"
if "%CI_BUILD_GUI%"=="TRUE" if not defined QT_ROOT_DIR (
  echo ERROR: QT_ROOT_DIR is required for a GUI build
  endlocal & exit /B 1
)

cmake.exe -S "%ZANO_SOURCE_DIR%" -B "%ZANO_BUILD_DIR%" ^
  -G "Visual Studio 17 2022" ^
  -A "%ZANO_CMAKE_ARCH%" ^
  -T "v143,host=x64" ^
  "-DTESTNET=%CI_TESTNET%" ^
  "-DBUILD_GUI=%CI_BUILD_GUI%" ^
  -DBUILD_TESTS=FALSE ^
  -DUSE_PCH=FALSE ^
  -DSTATIC=FALSE ^
  ...
  "-DBOOST_ROOT=%ZANO_DEPS_DIR%\boost" ^
  "-DBOOST_LIBRARYDIR=%ZANO_DEPS_DIR%\boost\lib" ^
  "-DOPENSSL_ROOT_DIR=%ZANO_DEPS_DIR%\openssl"
if errorlevel 1 (
...
set "CI_TARGETS=daemon simplewallet connectivity_tool gateway_rpc_proxy"
if /I "%ZANO_BUILD_GUI%"=="true" set "CI_TARGETS=Zano %CI_TARGETS%"
if "%ZANO_BUILD_GUI%"=="1" set "CI_TARGETS=Zano %CI_TARGETS%"
if not defined ZANO_BUILD_PARALLEL set "ZANO_BUILD_PARALLEL=2"

cmake.exe --build "%ZANO_BUILD_DIR%" ^
  --config Release ^
  --parallel "%ZANO_BUILD_PARALLEL%" ^
  --target %CI_TARGETS%
if errorlevel 1 (
...
:ci_require
if not defined %~1 (
  echo ERROR: %~1 is required for CI mode
  exit /B 1
)
exit /B 0
