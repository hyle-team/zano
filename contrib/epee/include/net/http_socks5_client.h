...
public:
  ...
  { return connect_impl(host, port, _default_timeout); }
...
private:
  ...
  bool connect_impl(const std::string& dest_host, const std::string& dest_port_s, unsigned int timeout)
  {
    ...
    if (!http_universal_client::connect(_proxy_host, std::to_string(_proxy_port), timeout ? timeout : _default_timeout))
      return false;
...
private:
  ...
  unsigned int _default_timeout = 15000;
};
