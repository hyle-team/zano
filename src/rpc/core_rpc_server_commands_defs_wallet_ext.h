...
namespace currency
{
  ...
  {
    DOC_COMMAND("Retrieves the transaction history for a specific gateway address (decrypted server-side, requires the view secret key).");
  ...
  struct COMMAND_RPC_GATEWAY_GET_ADDRESS_HISTORY2
  {
    DOC_COMMAND("Retrieves the public transaction history for a gateway address plus the raw transactions, for client-side decryption (no view secret key is sent to the daemon).");
    ...
    struct response
    {
      ...
      std::string status_error;
      uint64_t total_transactions = 0;
      std::list<tools::wallet_public::wallet_transfer_info> transactions; // public part only (not decrypted)
      std::list<gateway_balance_entry> balances;
      ...
      BEGIN_KV_SERIALIZE_MAP()
        ...
        KV_SERIALIZE(status_error)               DOC_DSCR("Error description if the call failed.") DOC_END
        KV_SERIALIZE(total_transactions)         DOC_DSCR("Total number of transactions associated with the specified gateway address.") DOC_EXMP(100) DOC_END
        KV_SERIALIZE(transactions)               DOC_DSCR("Public part of the transactions (NOT decrypted). Decrypt payment_id/attachments locally from raw_txs.") DOC_EXMP_AUTO(1) DOC_END
        KV_SERIALIZE(balances)                   DOC_DSCR("List of balances for different asset_id associated with the gateway address at the time of the request.") DOC_EXMP_AUTO(1) DOC_END
        KV_SERIALIZE(raw_txs)                    DOC_DSCR("Hex-serialized raw transactions, aligned 1:1 with 'transactions', for client-side decryption.") DOC_END
      END_KV_SERIALIZE_MAP()
    };
  };
