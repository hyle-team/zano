...
#ifndef TESTNET                                                                        // 1/4
  static_assert(ZANO_HARDFORK_06_AFTER_HEIGHT == 999999999999999999);                  // 2/4
#endif                                                                                 // 3/4
  if (height > 20445)                                                                  // 4/4 TODO: remove these 4 lines before releasing 2.2.x for mainnet -- sowle
  {
...
bool blockchain_storage::validate_tx_for_hardfork_specific_terms(const transaction& tx, const crypto::hash& tx_id, uint64_t block_height) const
{
  ...
  {    
    ...
    // pure gw txs: all in = txin_gateway, all out = tx_out_gateway or none
    const bool is_pure_gw_tx_hf6 = m_core_runtime_config.is_hardfork_active_for_height(6, block_height) &&
      !tx.vin.empty()
      &&
      std::all_of(tx.vin.begin(), tx.vin.end(), [](const txin_v& i) { return i.type() == typeid(txin_gateway); })
      &&
      std::all_of(tx.vout.begin(), tx.vout.end(), [](const tx_out_v& o) { return o.type() == typeid(tx_out_gateway); });
...
bool blockchain_storage::collect_rangeproofs_data_from_tx(const transaction& tx, const crypto::hash& tx_id, std::vector<zc_outs_range_proofs_with_commitments>& agregated_proofs)
{
  ...
  {
    bool all_gateway_outs = !tx.vout.empty() && std::all_of(tx.vout.begin(), tx.vout.end(), [](const tx_out_v& o) { return o.type() == typeid(tx_out_gateway); });
    if (tx.vout.empty() || all_gateway_outs)
    {
