...
//     }
  ...
  bool core::handle_block_found(const block& b, block_verification_context* p_verification_result, bool need_update_miner_block_template)
  {
    try
    {
      ...
      {
        LOG_PRINT2(CURRENCY_FAILED_MINED_BLOCKS_LOG_FILENAME, "block " << get_block_hash(b) << " @ height " << get_block_height(b) << ", verification_failed: " << p_verification_result->m_verification_failed << ", added_to_main_chain: " << p_verification_result->m_added_to_main_chain << ENDL <<
