...
#pragma GCC diagnostic pop
  ...
  bool calculate_html_folder_hash(const std::string& html_path, std::string& result_hash)
  {
    ...
    while (it.hasNext())
    {
      ...
      // normalize to forward slashes to match cmake file(GLOB_RECURSE RELATIVE ...) behavior
      rel.replace('\\', '/');
      std::string rel_str = rel.toStdString();
      // skip the same extensions like in html_content_hash.cmake
      if (rel_str.size() >= 4)
      {
