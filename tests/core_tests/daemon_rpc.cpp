...
bool fill_tx_rpc_inputs::generate(std::vector<test_event_entry>& events) const
{
  ...
  DO_CALLBACK_PARAMS_STR(events, "c2", t_serializable_object_to_blob(currency::transaction{/* .version = */ 0,
                                                                                           /* .vin = */ {/* [0] = */ currency::txin_to_key{}}}));

  // A transaction with an input of the type "txin_to_key" in the default state.k
  DO_CALLBACK_PARAMS_STR(events, "c6", t_serializable_object_to_blob(currency::transaction{/* .version = */ 0,
                                                                                           /* .vin = */ {/* [0] = */ currency::txin_zc_input{}}}));

  // A transaction with several "txin_to_key" inputs those have different .amount values.
  DO_CALLBACK_PARAMS_STR(events, "c3", t_serializable_object_to_blob(currency::transaction{/* .version = */ 0,
                                                                                           /* .vin = */ {/* [0] = */ currency::txin_to_key{{}, 0},
                                                                                                         /* [1] = */ currency::txin_to_key{{}, UINT64_MAX},
                                                                                                         /* [2] = */ currency::txin_to_key{{}, 1}}}));

  // A transaction with inputs in the default states of all possible types.
  DO_CALLBACK_PARAMS_STR(events, "c4", t_serializable_object_to_blob(currency::transaction{/* .version =*/ 0,
                                                                                           /* .vin = */ {/* [0] = */ currency::txin_gen{},
                                                                                                         /* [1] = */ currency::txin_to_key{},
                                                                                                         /* [2] = */ currency::txin_htlc{},
                                                                                                         /* [3] = */ currency::txin_zc_input{},
                                                                                                         /* [4] = */ currency::txin_multisig{}}}));
  ...
  // A transaction with inputs of all possible types.
  {
    ...
    tx.vin.emplace_back(currency::txin_gen{/* .height = */ 7137406440025745250});

    {
      currency::txin_to_key input{};

      input.key_offsets.emplace_back(5350230927837587142ull);
      input.key_offsets.emplace_back(currency::ref_by_id{currency::get_transaction_hash(tx_0), 0u});
      ...
      input.etc_details.emplace_back(currency::signed_parts{/* .n_outs = */ 2613407258u, /* .n_extras = */ 347754399u});
      input.etc_details.emplace_back(currency::extra_attachment_info{/* .sz = */ 5559118977069213037ull, /* .hsh = */ currency::null_hash, /* cnt = */ 8106306627691316520ull});
...

    {
      ...
      input.key_offsets.emplace_back(9536097715806449708ull);
      input.key_offsets.emplace_back(currency::ref_by_id{/* .tx_id = */ currency::get_transaction_hash(tx_0), /* .n = */ 9u});
      ...
      input.etc_details.emplace_back(currency::signed_parts{/* .n_outs = */ 517318753u, /* .n_extras = */ 1367922888u});
      input.etc_details.emplace_back(currency::extra_attachment_info{/* .sz = */ 10987762797757012676ull, /* .hsh = */ currency::null_hash, /* .cnt = */ 10767056422067827733ull});
...

    {
      ...
      input.key_offsets.emplace_back(16540286509618649069ull);
      input.key_offsets.emplace_back(currency::ref_by_id{/* .tx_id = */ currency::get_transaction_hash(tx_0), /* .n = */ 4});
...

    {
      ...
      input.etc_details.emplace_back(currency::signed_parts{/* .n_outs = */ 1801772931u, /* .n_extras = */ 167800219u});
      input.etc_details.emplace_back(currency::extra_attachment_info{/* .sz = */ 12438971857615319230ull, /* .hsh = */ currency::null_hash, /* .cnt = */ 13515222808659969031ull});
      tx.vin.push_back(input);
    }
  ...
  // A wrong reference by an object of the type "ref_by_id": a value of the attribute "n" is greater that a size of the outputs container. The function must "fill_tx_rpc_inputs" return false.
  {
    currency::transaction tx{};

    {
      ...
      input.key_offsets.emplace_back(currency::ref_by_id{currency::get_transaction_hash(tx_0), 1968482779u});
      tx.vin.push_back(input);
    }
  ...
  // A wrong reference by an object of the type "ref_by_id": a value of the attribute "tx_id" indicates a non-existent transaction. The function must "fill_tx_rpc_inputs" return false.
  {
    currency::transaction tx{};

    {
      ...
      input.key_offsets.emplace_back(currency::ref_by_id{currency::null_hash, 0u});
      tx.vin.push_back(input);
    }
