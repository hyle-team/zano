...
bool pos_and_no_pow_blocks_between_output_and_stake::generate(std::vector<test_event_entry>& events) const
{
  /*
    ...
    Script:
    - A PoW chain is created up to a height of 24s
  ...
  */
  ...
  REWIND_POS_BLOCKS_N(events, blk_3r, blk_2r, miner, { miner }, CURRENCY_MINED_MONEY_UNLOCK_WINDOW);
  ...
  REWIND_POS_BLOCKS_N(events, blk_3ra, blk_1a, miner, { miner }, CURRENCY_MINED_MONEY_UNLOCK_WINDOW - 1);
